odoo.define('s_media_block.snippets.editor', function (require) {
'use strict';
    ...
    ajax.loadXML('/s_media_block/static/src/xml/s_media_block_modal.xml', qweb);

    // MEDIABLOCK
    options.registry.s_media_block = options.Class.extend({
        ...
        video_opt: function (type, value, $li) {
            if (type !== 'click')  return;
            var self = this;
            self.$modal = $(qweb.render("s_media_block.s_media_block_modal"));
            ...
            $muted.on('click', function() {
                ($muted.attr('checked')) ? $muted.attr('checked', false) : $muted.attr('checked', true);
            });
            $loop.on('click', function() {
                ($loop.attr('checked')) ? $loop.attr('checked', false) : $loop.attr('checked', true);
            });
            $controls.on('click', function() {
                ($controls.attr('checked')) ? $controls.attr('checked', false) : $controls.attr('checked', true);
            });
            $autoplay.on('click', function() {
                ($loop.attr('checked')) ? $autoplay.attr('checked', false) : $autoplay.attr('checked', true);
            });
